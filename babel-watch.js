...
const loadWorkerThreads = _.once(function _loadWorkerThreads() {
  try {
...
const { Worker, isMainThread, parentPort, workerData } = loadWorkerThreads();
...
if (isMainThread) {
  ...
  program.option(
    "--threads",
    "Use Worker Threads to transpile Babel source (Experimental)"
  );
  program.option(
    "-i, --ignore [globs]",
    "Matching files will not be transpiled"
  );
  program.option(
  ...
  const debouncedRestart = _.debounce(restartApp, 100);
    ...
    if (childApp) {
      ...
      childApp.on("exit", () => {
        ...
        if (currentPipeFilename) {
          fs.unlink(currentPipeFilename, _.noop); // silently remove old pipe file - ignore callback
        }
        restartAppInternal();
      });
      try {
  ...
  function restartAppInternal() {
    ...
    app.on("message", data => {
      ...
      handleFileLoad(filename, (source, sourceMap) => {
        const sourceBuf = source ? Buffer.from(source, "utf8") : null;
        const mapBuf = sourceMap
          ? Buffer.from(JSON.stringify(sourceMap), "utf8")
          : null;
        const lenBuf = Buffer.alloc(4);
        try {
          lenBuf.writeUInt32BE((sourceBuf && sourceBuf.length) || 0, 0);
          fs.writeSync(pipeFd, lenBuf, 0, 4);
          sourceBuf && fs.writeSync(pipeFd, sourceBuf, 0, sourceBuf.length);

          lenBuf.writeUInt32BE((mapBuf && mapBuf.length) || 0, 0);
          fs.writeSync(pipeFd, lenBuf, 0, 4);
          mapBuf && fs.writeSync(pipeFd, mapBuf, 0, mapBuf.length);
        } catch (error) {
          // EPIPE means `pipeFd` has been closed. We can ignore this
          if (
            error.code !== "EPIPE" &&
            !/first argument must be file descriptor/i.test(error.toString()) &&
            !/argument must be of type number/i.test(error.toString())
          ) {
            throw error;
          }
        }
  ...
  const transformOpts = workerData.transformOpts;
  ...
  const result = babel.transformFileSync(filename, opts);
